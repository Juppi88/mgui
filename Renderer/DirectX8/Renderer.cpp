...
private:
	...
	IDirect3DSurface8*	oldDepthStencil;
...
IDirect3DTexture8*		CRenderer::renderTexture	= NULL;			// Current texture
IDirect3DVertexBuffer8*	CRenderer::vertexBuffer		= NULL;			// Pointer to vertex buffer
IDirect3DVertexBuffer8*	CRenderer::vertexBuffer3d	= NULL;			// Pointer to 3D vertex buffer
...
DWORD					CRenderer::normState		= 0;			// Norm state
DWORD					CRenderer::drawState		= 0;			// Draw state
DWORD					CRenderer::drawState3d		= 0;			// Draw state for 3D mode
...
D3DVIEWPORT8			CRenderer::viewport;						// Default viewport
...
void CRenderer::Initialize( void )
{
	...
	d3dDevice->CreateVertexBuffer( MAX_VERT * sizeof(Vertex), D3DUSAGE_WRITEONLY|D3DUSAGE_DYNAMIC, D3DFVF_TEXTURED, D3DPOOL_DEFAULT, &vertexBuffer );
	d3dDevice->CreateVertexBuffer( MAX_VERT * sizeof(Vertex3D), D3DUSAGE_WRITEONLY|D3DUSAGE_DYNAMIC, D3DFVF_TEXTURED_3D, D3DPOOL_DEFAULT, &vertexBuffer3d );
...
void CRenderer::Shutdown( void )
{
	...
	d3dDevice->DeleteStateBlock( normState );
	d3dDevice->DeleteStateBlock( drawState );
	d3dDevice->DeleteStateBlock( drawState3d );
}
...
void CRenderer::Begin( void )
{
	...
	d3dDevice->CaptureStateBlock( normState );
...
void CRenderer::End( void )
{
	...
	d3dDevice->ApplyStateBlock( normState );
...
void CRenderer::StartClip( int32 x, int32 y, uint32 w, uint32 h )
{
	...
	// TODO: Eventually (?) make this work with more than just text.
...
MGuiRendTarget* CRenderer::CreateRenderTarget( uint32 width, uint32 height )
{
	...
	cache->oldDepthStencil = NULL;
	res = d3dDevice->CreateTexture( w, h, 1, D3DUSAGE_RENDERTARGET, D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT, &cache->texture );
...
void CRenderer::EnableRenderTarget( const MGuiRendTarget* target, int32 x, int32 y )
{
	...
	d3dDevice->GetRenderTarget( &cache->oldSurface );
	d3dDevice->GetDepthStencilSurface( &cache->oldDepthStencil );

	if ( cache->oldSurface != cache->surface )
	{
		d3dDevice->SetRenderTarget( cache->surface, cache->oldDepthStencil );
...
void CRenderer::DisableRenderTarget( const MGuiRendTarget* target )
{
	...
	{
		d3dDevice->SetRenderTarget( cache->oldCache->surface, cache->oldDepthStencil );
		SAFE_RELEASE( cache->oldDepthStencil );
	...
	d3dDevice->SetRenderTarget( cache->oldSurface, cache->oldDepthStencil );
	SAFE_RELEASE( cache->oldDepthStencil );
...
void CRenderer::BeginDraw( void )
{
	...
	{
		vertexBuffer->Lock( 0, 0, (BYTE**)&vertex, D3DLOCK_DISCARD );
		lockBuffer = false;
	}

	if ( !vertex3d && lockBuffer3d )
	{
		vertexBuffer3d->Lock( 0, 0, (BYTE**)&vertex3d, D3DLOCK_DISCARD );
		lockBuffer3d = false;
	}
}
...
void CRenderer::EndDraw( void )
{
	...
	case DRAWING_3D:
		d3dDevice->ApplyStateBlock( drawState3d );
		break;

	default:
		d3dDevice->ApplyStateBlock( drawState );
		break;
	}
	...
	if ( numVertices )
	{
		...
		d3dDevice->GetVertexShader( &fvfOld );
		...
		d3dDevice->SetVertexShader( fvfOld );
	}
	else if ( numVertices3d )
	{
		...
		d3dDevice->SetVertexShader( fvf );
		d3dDevice->SetTexture( 0, renderTexture );
		d3dDevice->SetStreamSource( 0, vertexBuffer3d, sizeof(Vertex3D) );
...
void CRenderer::CreateFontTexture( void* fontData, uint32 width, uint32 height, void** texture, uint32* texturePitch )
{
	...
	hr = d3dDevice->CreateTexture( width, height, 1, 0, D3DFMT_A4R4G4B4, D3DPOOL_MANAGED, &font->texture );
	if ( FAILED( hr ) ) return;
